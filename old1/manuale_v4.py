...
class ManualeFrame(tk.Frame):
    def __init__(self, master, on_back, get_axis_values, on_manual_command=None, *args, **kwargs):
        ...
        self.z_active = False
        self.xy_poll_after = None
        self.z_poll_after = None
    ...
    def build_ui(self):
        ...
        hint = tk.Label(
            bottom,
            text="Tocca e trascina. La posizione viene letta in polling continuo, più affidabile sul touchscreen.",
            font=("Arial", 18),
            fg="#cfcfcf",
            bg="#101010",
        )
    ...
    def xy_release(self, event):
        self.xy_active = False
        if self.xy_poll_after is not None:
            self.after_cancel(self.xy_poll_after)
            self.xy_poll_after = None
    ...
    def z_release(self, event):
        self.z_active = False
        if self.z_poll_after is not None:
            self.after_cancel(self.z_poll_after)
            self.z_poll_after = None
    ...
    def xy_poll(self):
        ...
        local_x = px - self.xy_canvas.winfo_rootx()
        local_y = py - self.xy_canvas.winfo_rooty()
        self.update_xy_from_pointer(local_x, local_y)
        self.xy_poll_after = self.after(20, self.xy_poll)

    def z_poll(self):
        if not self.z_active:
            return
        px, py = self.winfo_pointerxy()
        local_y = py - self.z_canvas.winfo_rooty()
        self.update_z_from_pointer(local_y)
        self.z_poll_after = self.after(20, self.z_poll)
