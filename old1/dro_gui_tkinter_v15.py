...
class DROApp(tk.Tk):
    def __init__(self):
        ...
        # Correzione scala: il Teensy/LS7366 invia incrementi di 4 conteggi
        # per ogni passo reale da 0,005 mm. Quindi 1 conteggio letto vale 0,00125 mm.
        ...
        self.max_reasonable_count_jump = 20000.0
    ...
    def handle_serial_line(self, line):
        ...
        for part in line.split(","):
            ...
            raw = raw.strip()
            if axis in ("X", "Y", "Z"):
                count = self.parse_count(raw)
                if count is not None:
                    if (not self.have_valid_count[axis] or
                            abs(count - self.raw_counts[axis]) <= self.max_reasonable_count_jump):
                        self.raw_counts[axis] = count
